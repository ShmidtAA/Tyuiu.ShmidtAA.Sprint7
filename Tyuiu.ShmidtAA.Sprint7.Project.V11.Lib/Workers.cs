...
namespace Tyuiu.ShmidtAA.Sprint7.Project.V11.Lib
{
    ...
    public struct Workers
    {
        ...
        /// <summary>
        /// Возраст
        /// Это свойство автоматически вычисляет возраст на основе даты рождения (_dateOfBirth).
        /// Возраст обновляется динамически при каждом обращении к свойству, что исключает необходимость
        /// вручную изменять значение, например, при переходе на новый год.
        /// </summary>
        public int Age
        {
            get
            {
                ...
                // Если дата рождения (_dateOfBirth.Date) еще не наступила в этом году,
                // то вычитаем 1 год из рассчитанного возраста для корректировки.
                if (_dateOfBirth.Date > today.AddYears(-age))
                {
                    age--; // Уменьшаем возраст на 1, если день рождения еще не прошел.
                }

                // Возвращаем корректно вычисленный возраст.
                return age;
            }
        }
        /// <summary>
